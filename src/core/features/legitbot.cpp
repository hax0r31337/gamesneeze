...
void Features::LegitBot::createMove(CUserCmd* cmd) {
    if (!CONFIGBOOL("Legit>LegitBot>Enabled") || !(Menu::CustomWidgets::isKeyDown(CONFIGINT("Legit>LegitBot>Default>Key")) || CONFIGINT("Legit>LegitBot>Default>Key") == 0) ||
    ...
    // Enumerate over players and get angle to the closest player hitbox
    // (that is enabled) to crosshair
      ...
      if (p && p != Globals::localPlayer) {
        if (p->health() > 0 && !p->dormant() && p->isEnemy() && p->visible()) {
          ...
          if (p->getAnythingBones(boneMatrix)) {
            ...
            if (model) {
              for (int i = 0; i < 5; i++) { // swap this for for loop of
                ...
                if (hitboxes & 1 << i) {
                  ...
                  QAngle angleToCurrentBone =
                      calcAngle(localPlayerEyePos, targetBonePos) -
                      cmd->viewangles -
                      (recoilCompensation ? Globals::localPlayer->aimPunch() * 2
                                          : QAngle(0, 0, 0));

...
void Features::AntiAim::createMove(CUserCmd* cmd) {
    if (Interfaces::engine->IsInGame()) {
        if (Globals::localPlayer) {
            if (Globals::localPlayer->health() > 0 && Globals::localPlayer->moveType() == MOVETYPE_WALK) { // 9 is ladder movetype
                            ...
                            if (CONFIGINT("Rage>AntiAim>Type")) {
                                ...
                                switch (CONFIGINT("Rage>AntiAim>Type")) {
                                    ...
                                    case 6: { // Legit
                                      ...
                                      if (realtime - lastTime > CONFIGINT("Rage>AntiAim>FakeLag") / 0.125) {
                                        invert = !invert;
                                        lastTime = realtime;
                                      }

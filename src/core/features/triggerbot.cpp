...
void Features::Triggerbot::createMove(CUserCmd* cmd) {
    if (!Globals::localPlayer || !CONFIGBOOL("Legit>Triggerbot>Triggerbot") || !(CONFIGINT("Legit>Triggerbot>Key") == 0 || Menu::CustomWidgets::isKeyDown(CONFIGINT("Legit>Triggerbot>Key")))) {
        return;
    }
    ...
    for (int i = 0; i < 100; i++) {
      ...
      Entity *ent = findPlayerThatRayHits(Globals::localPlayer->eyePos(),
                                          endPos, &traceToPlayer);
      if (ent && ent->clientClass()->m_ClassID == CCSPlayer &&
          !ent->dormant() && ((Player *)ent)->isEnemy()) {
